...
def check_email_or_phone(email_or_phone):
    # Avval emailni tekshiramiz
    if re.fullmatch(email_regex, email_or_phone):
        return "email"

    # Telefon raqam bo'lsa tekshirish
    if re.fullmatch(phone_regex, email_or_phone):
        try:
            ...
            pass  # Agar noto'g'ri format bo'lsa davom etadi

    # Email ham emas, telefon raqam ham emas bo'lsa xatolik chiqarish
    data = {
        "success": False,
...
def send_phone_number(phone, code):
    ...
    client.messages.create(
        body=f"SIzga jo'natilgan kod : {code} \n",
        from_="+998947320406",
        to = f"+{phone}"
